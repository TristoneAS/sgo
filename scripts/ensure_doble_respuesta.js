...
async function main() {
  ...
  if (!names.has("doble_respuesta")) {
    await conn.query(`
      ...
      AFTER tipo_dato
    `);
    console.log("Columna doble_respuesta agregada");
  }
  ...
  if (!names.has("etiqueta_1")) {
    ...
    `);
    console.log("Columna etiqueta_1 agregada");
  }
  ...
  if (!names.has("etiqueta_2")) {
    await conn.query(`
      ...
      AFTER etiqueta_1
    `);
    console.log("Columna etiqueta_2 agregada");
  }

  console.log("formato_columnas lista para doble respuesta");
  await conn.end();
}
